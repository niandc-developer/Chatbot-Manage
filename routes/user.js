...
// APIパス
var ApiPath = "v1/user";
var ApiPasswordPath = "v1/user/password";
...
router.get('/edit/:id', function(req, res) {
  ...
  request(options, function (error, response, body) {
    if (!error && response.statusCode == 200) {
      res.render("user/edit", { data : body[0] });
    } else if (!error && response.statusCode < 500) {
      body.href = req.headers.referer;
      res.render("warning", body);
    } else {
...
router.get('/change_password/:id', function(req, res) {
  ...
  request(options, function (error, response, body) {
    if (!error && response.statusCode == 200) {
      res.render("user/change_password", { data : body[0] });
    } else if (!error && response.statusCode < 500) {
      body.href = req.headers.referer;
      res.render("warning", body);
    } else {
...
router.post('/change_password', function(req, res) {
  var url = global.config.url + ApiPasswordPath;
